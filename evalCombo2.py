...
FOLD = STEM.split('/')[-1].replace('FOLD','')
...
def getOut(Data):
	...
	if ',' in Data[1]:
		delim = ','
	return [int(x.split(delim)[0]) for x in Data[1:]]
...
db.query('CREATE DATABASE IF NOT EXISTS MOSES')
db.query('CREATE TABLE IF NOT EXISTS MOSES.RESULTS(ID INT(10) PRIMARY KEY AUTO_INCREMENT, FILENAME VARCHAR(128), FOLD INT(10), MODEL LONGTEXT, _11 INT(10), _10 INT(10), _01 INT(10), _00 INT(10))')
...
#		pass
	os.system(etExec+' -i '+STEM+'.test -f '+STEM+'.eval.log'+' -C '+STEM+'_EVAL/M'+str(c)+'.model -u out -o '+STEM+'_RESULT/R'+"%03d"% c+'.result')
	PRD = [int(x.strip()) for x in open(STEM+'_RESULT/R'+"%03d"% c+'.result0').readlines()[1:]]
	...
	#print M
	#print ('INSERT INTO RESULTS(EXPERIMENT, BINSIZE, MODEL, _11, _10, _01, _00) VALUES (\''+sys.argv[1]+'\','+str(len(OBS))+',\''+M+'\''+str(CONFUSION[0])+','+str(CONFUSION[1])+','+str(CONFUSION[2])+','+str(CONFUSION[3])+')')
	db.query('INSERT INTO MOSES.RESULTS(FILENAME, FOLD, MODEL, _11, _10, _01, _00) VALUES (\''+FILENAME+'\','+FOLD+',\''+M+'\','+str(CONFUSION[0])+','+str(CONFUSION[1])+','+str(CONFUSION[2])+','+str(CONFUSION[3])+')')
